...
else
{
	...
	$wmax=200;
	$hmax=150;
	$bg_image_name="$folderpath/$fileName";
	$bg_image_select_query=mysql_query("SELECT * FROM customize where id='$id'");
	$count_rows=mysql_num_rows($bg_image_select_query);
	if($count_rows==0)
	{
		$bg_image_insert_query=mysql_query('INSERT into customize values("'.$id.'","#ffffff","#2D002D","'.$bg_image_name.'")');
	}
	else
		$bg_image_update_query=mysql_query("UPDATE customize SET bg_image='$bg_image_name' WHERE id='$id'");
	
	$img_r = imagecreatefromjpeg($bg_image_name);
...
<div id="main-box">
       ...
       <div id="overlay-bg">
     		<div id="overlay-inside-bg">
                ...
                <button type="submit" class="button" id="upload2" value="Upload">Upload</button>
                <input type="hidden" name="question" value=<?php echo $prev?>>
                <?php
				if($prev==1)
				{
          			echo "<button type=button class=button id=close-prev-4 value=Close>Close</button>";
				}
				else if($prev==2)
				{
					echo "<button type=button class=button id=close-prev-5 value=Close>Close</button>";
				}
				else if($prev==3)
				{
					echo "<button type=button class=button id=close-prev-6 value=Close>Close</button>";
				}
				?>
