...
<div id="main">
	<section>
		...
		<div id="right-box">
        	<p class="labels" id="tags">Responses</p>
            <div id="results">
            <?php
			$ResponseList="";
			$id2="";
			$blank_img='images/blank.jpg';
			$findResponses='SELECT * FROM responses WHERE id_from="'.mysql_real_escape_string($id).'" ORDER BY time DESC';
			$result=mysql_query($findResponses) or die(mysql_error());
			...
			{	while($row=mysql_fetch_array($result))
				{
					...
					$response_info='SELECT * from users where id="'.$id_to.'"';
					$result1=mysql_query($response_info) or die(mysql_error());
					...
					if($totalrows2>0)
						while($row=mysql_fetch_array($result1))
						{
							...
							$hisher='his';
								...
								if($response_type=='Request-Response')
								{
									if($response=='Accepted')
									{
										if($pic=='')
										{
										$ResponseList[] .='<ul class="link-list search-list"><li><a href="profile-view.php?id='.$id3.'"><img class="search-pic1" src='.$blank_img.' height="35" width="35"/>'.$firstname.'  '.$lastname.'</a><p>The above person has added you to '.$hisher.' contacts.<br /><br /><br /><br /><span class="time-span-class">@ '.$date.'</span></p></li></ul>';
										}
										else
										{
										$ResponseList[] .='<ul class="link-list search-list"><li><a href="profile-view.php?id='.$id3.'"><img class="search-pic1" src='.$pic.' height="35" width="35"/>'.$firstname.'  '.$lastname.'</a><p>The above person has added you to '.$hisher.' contacts.<br /><br /><br /><br /><span class="time-span-class">@ '.$date.'</span></p></li></ul>';
										}
									}
								}
								else if($response_type=='Blab-Comment')
								{
									$blab_query='SELECT * from blabs where blab_id="'.$response.'"';
									$blabresult=mysql_query($blab_query) or die(mysql_error());
									while($blabrow=mysql_fetch_array($blabresult))
									{
										$blab=$blabrow['blab_content'];
									}
										if($pic=='')
										{
										$ResponseList[] .='<ul class="link-list search-list"><li><a href="profile-view.php?id='.$id3.'"><img class="search-pic1" src='.$blank_img.' height="35" width="35"/>'.$firstname.'  '.$lastname.'</a><p>The above person commented on your blab:<br /><br /><br /><br />"'.stripslashes(wordwrap(nl2br($blab), 75, "<br /><br /><br /><br />", true)).'"<br /><br /><br /><br /><span class="time-span-class">@ '.$date.'</span></p><form method="post" action="view-response.php"><div id="accept-reject-buttons"><input type="submit" class="newbutton" value="View" /><input type="hidden" name="blab-id" value="'.$response.'" /></div></form></li></ul>';
										}
										else
										{
										$ResponseList[] .='<ul class="link-list search-list"><li><a href="profile-view.php?id='.$id3.'"><img class="search-pic1" src='.$pic.' height="35" width="35"/>'.$firstname.'  '.$lastname.'</a><p>The above person commented on your blab:<br /><br /><br /><br />"'.stripslashes(wordwrap(nl2br($blab), 75, "<br /><br /><br /><br />", true)).'"<br /><br /><br /><br /><span class="time-span-class">@ '.$date.'</span></p><form method="post" action="view-response.php"><div id="view-button"><input type="submit" class="newbutton" value="View" /><input type="hidden" name="blab-id" value="'.$response.'" /></div></form></li></ul>';
										}
								}
				}
				//Pagination
				...
				if($start==0){
					...
					{
						if(array_key_exists($i, $ResponseList))
						echo $ResponseList[$i];
						else
						break;
					}
...
<?php
				...
				{$ResponseList.='<p class="p-class">You Have No Responses.</p>';
		 		echo $ResponseList;}
?>
            </ul>
            </div>
