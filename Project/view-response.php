...
$comm_date="";
$timestamp="";
$blabid=$_POST['blab-id'];
  
$blabquery='SELECT * FROM blabs where blab_id="'.$blabid.'"';
$blabresult=mysql_query($blabquery);
while($row = mysql_fetch_assoc($blabresult)){
	...
	while($row = mysql_fetch_array($sql_mem_data)){
		...
		$sql_comments=mysql_query("SELECT * FROM comments WHERE id_blab=$blabid ORDER BY comm_date ASC ");
		$count=mysql_query("SELECT COUNT(*) FROM comments WHERE id_blab=$blabid ORDER BY comm_date");
		while($row1=mysql_fetch_array($sql_comments))
		    {
				...
				$display_comments.='<div class="comments-class"><ul class="link-list search-list"><li><a href="profile.php"><img class="search-pic" src='.$commentator_pic.' />'.$f_name.' '.$l_name.'</a><p class="blabs-p">'.$comments_list_formatted.'<br /><span class="time-span-class">@ '.$comm_date.'</span></p><div id="delete-comment-button"><button type="button" id="'.$comm_id.'" class="delete_comments" value="Delete">Delete</button></div></li></ul></div>';
					}
					else
					{
					$comments_list_formatted=stripslashes(wordwrap(nl2br($comments_list), 65, "<br>", true));
				$display_comments.='<div class="comments-class"><ul class="link-list search-list"><li><a href="profile-view.php?id='.$commentator.'"><img class="search-pic" src='.$commentator_pic.' />'.$f_name.' '.$l_name.'</a><p class="blabs-p">'.$comments_list_formatted.'<br /><span class="time-span-class">@ '.$comm_date.'</span></p></li></ul></div>';
					}
				}
...
'.$display_comments.'
</div><form  method="POST" action=""><input type="text" id="commentcontent'.$blabid.'" class="text-box"/> <input type="button" id="'.$blabid.'" class="newbutton" value="Post Comment" /> <button type="button" class="button_delete" id="'.$blabid.'" value="Delete">Delete</button><input type="hidden" name="blabid" id="blabid" value="'.$blabid.'"/></form>
</form></div></li></ul>';
...
<div id="comments-box'.$blabid.'">
'.$display_comments.'
</div><form method="POST" action=""><input type="text" id="commentcontent'.$blabid.'" class="text-box"/> <input type="button" class="newbutton" id="'.$blabid.'" value="Post Comment"/><input type="hidden" name="blabid" id="'.$blabid.'" value="'.$blabid.'"/></form>
 </form></div></li></ul>';
